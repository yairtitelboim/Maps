...
const PERIODS = [
  { id: '2017_2018', label: '2017 → 2018', file: '/data/samsung_taylor_wastewater/samsung_taylor_wastewater_2017_2018.geojson' },
  { id: '2018_2019', label: '2018 → 2019', file: '/data/samsung_taylor_wastewater/samsung_taylor_wastewater_2018_2019.geojson' },
  { id: '2019_2020', label: '2019 → 2020', file: '/data/samsung_taylor_wastewater/samsung_taylor_wastewater_2019_2020.geojson' },
  { id: '2020_2021', label: '2020 → 2021', file: '/data/samsung_taylor_wastewater/samsung_taylor_wastewater_2020_2021.geojson' },
  { id: '2021_2022', label: '2021 → 2022', file: '/data/samsung_taylor_wastewater/samsung_taylor_wastewater_2021_2022.geojson' },
  { id: '2022_2023', label: '2022 → 2023', file: '/data/samsung_taylor_wastewater/samsung_taylor_wastewater_2022_2023.geojson' },
  { id: '2023_2024', label: '2023 → 2024', file: '/data/samsung_taylor_wastewater/samsung_taylor_wastewater_2023_2024.geojson' },
  { id: '2024_2025', label: '2024 → 2025', file: '/data/samsung_taylor_wastewater/samsung_taylor_wastewater_2024_2025.geojson' }
];

const SAMSUNG_CENTER = [-97.3514, 30.5706];
const SAMSUNG_RADIUS_METERS = 7200;

const LAYER_IDS = {
  source: 'samsung-taylor-geo',
  aoiFill: 'samsung-taylor-aoi-fill',
  aoiOuter: 'samsung-taylor-aoi-outer',
  industrialFill: 'samsung-taylor-industrial-fill',
  roads: 'samsung-taylor-roads',
  power: 'samsung-taylor-power',
  siteLabel: 'samsung-taylor-site-label'
};

const SAMSUNG_RING_IDS = {
  circleSource: 'samsung-taylor-ring-source',
  haloInnerSource: 'samsung-taylor-ring-halo-inner-source',
  haloOuterSource: 'samsung-taylor-ring-halo-outer-source',
  sparkleSource: 'samsung-taylor-ring-sparkles-source',
  circleFill: 'samsung-taylor-ring-fill',
  circleOutline: 'samsung-taylor-ring-outline',
  haloInnerLayer: 'samsung-taylor-ring-halo-inner',
  haloOuterLayer: 'samsung-taylor-ring-halo-outer',
  sparkleLayer: 'samsung-taylor-ring-sparkles'
};
...
const CHANGE_COLOR_RAMP = [
  ...
  'water_change', '#6366f1',
  'stable_background', 'rgba(148, 163, 184, 0.35)',
  '#94a3b8'
];
...
const CHANGE_COLOR_RAMP_SOFT = [
  ...
  'water_change', '#c7d2fe',
  'stable_background', 'rgba(226, 232, 240, 0.35)',
  '#e2e8f0'
];
...
const calculateChangeCallouts = (geojsonData) => {
  ...
  geojsonData.features.forEach((feature) => {
    ...
    if (
      changeType &&
      changeType !== 'persistent_agriculture' &&
      changeType !== 'stable_background' &&
      CHANGE_CALLOUTS[changeType]
    ) {
...
const SamsungTaylorChangeAnimation = ({ map, data, changeData = null, visible = true }) => {
  // Archived: Texas-specific animation - disabled for Columbus migration
  // This animation is for Samsung Taylor, TX and should not run for Columbus, OH
  return null;
  ...
  const applyTimelineLegendHighlight = useCallback((seriesKey) => {
    const normalizedKey = seriesKey && CHANGE_CALLOUTS[seriesKey] ? seriesKey : null;
    highlightedSeriesRef.current = normalizedKey;

    const m = map?.current;
    if (!m) return;

    const fillColorExpression = normalizedKey
      ? [
          'case',
          ['==', ['get', 'change_label'], normalizedKey],
          WATER_COLOR_RAMP,
          'rgba(17, 24, 39, 0.28)'
        ]
      : WATER_COLOR_RAMP;

    const lineColorExpression = normalizedKey
      ? [
          'case',
          ['==', ['get', 'change_label'], normalizedKey],
    ...
    activeLayersRef.current.forEach(({ fillId, lineId }) => {
      if (fillId && m.getLayer(fillId)) {
        m.setPaintProperty(fillId, 'fill-extrusion-color', fillColorExpression);
      }
      if (lineId && m.getLayer(lineId)) {
        m.setPaintProperty(lineId, 'line-color', lineColorExpression);
      }
    });
  }, [map]);

  useEffect(() => {
    applyTimelineLegendHighlight(timelineHighlightKey);
  }, [timelineHighlightKey, applyTimelineLegendHighlight]);

  useEffect(() => {
    applyTimelineLegendHighlight(highlightedSeriesRef.current);
  }, [layerGeneration, applyTimelineLegendHighlight]);

  useEffect(() => {
    if (typeof window === 'undefined' || !window.mapEventBus?.on) return undefined;

    const handleTimelineLegendFocus = (payload) => {
      if (!payload) return;
      const nextKey = payload.seriesKey && CHANGE_CALLOUTS[payload.seriesKey]
        ? payload.seriesKey
        : null;
      setTimelineHighlightKey((prev) => (prev === nextKey ? prev : nextKey));
    };

    const unsubscribe = window.mapEventBus.on('timeline:legendFocus', handleTimelineLegendFocus);

    return () => {
      unsubscribe?.();
      highlightedSeriesRef.current = null;
      applyTimelineLegendHighlight(null);
    };
  }, [applyTimelineLegendHighlight]);

  const playbackDelay = 675;
  ...
  const addCalloutLayers = useCallback((callouts) => {
    ...
    const sourceId = 'samsung-taylor-callouts-source';
    const layerId = 'samsung-taylor-callouts-labels';
  ...
  const removeChangeLayers = useCallback(() => {
    ...
    activeLayersRef.current.forEach(({ fillId, lineId, shadowId, sourceId, backgroundSourceId, backgroundFillId }) => {
      safeRemove(m, fillId);
      safeRemove(m, lineId);
      safeRemove(m, shadowId);
      safeRemove(m, sourceId, false);
      if (backgroundFillId) {
        safeRemove(m, backgroundFillId);
      }
      if (backgroundSourceId) {
        safeRemove(m, backgroundSourceId, false);
      }
    });
  ...
  const animateLayerOpacity = useCallback((fillId, targetOpacity, duration = 300) => {
    const m = map?.current;
  ...
  const addPeriodToMap = useCallback((period, periodIndex = 0) => {
    ...
    const sourceId = `samsung-taylor-change-${baseId}-source`;
    ...
    const backgroundSourceId = `${sourceId}-background`;
    const backgroundFillId = `${sourceId}-background-fill`;
    if (!m.getSource(backgroundSourceId)) {
      m.addSource(backgroundSourceId, {
        type: 'geojson',
        data: createCircleFeature(SAMSUNG_CENTER, SAMSUNG_RADIUS_METERS)
      });
    }
    if (!m.getLayer(backgroundFillId)) {
      m.addLayer({
        id: backgroundFillId,
        type: 'fill',
    ...
    setLayerGeneration((value) => value + 1);
  ...
  }, [map, animateLayerIn, animateOutlineGlow, animateShadowIn, addCalloutLayers, removeCalloutLayers, setLayerGeneration]);
  ...
  const addCircleLayer = useCallback(() => {
    ...
    const circleSourceId = 'samsung-taylor-site-circle-source';
    const haloInnerSourceId = 'samsung-taylor-site-halo-inner-source';
    const haloOuterSourceId = 'samsung-taylor-site-halo-outer-source';
    const sparkleSourceId = 'samsung-taylor-site-sparkles-source';

    const fillId = 'samsung-taylor-site-circle-fill';
    const outlineId = 'samsung-taylor-site-circle-outline';
    const haloInnerLayerId = 'samsung-taylor-site-halo-inner';
    const haloOuterLayerId = 'samsung-taylor-site-halo-outer';
    const sparkleLayerId = 'samsung-taylor-site-sparkles';

    addOrUpdateSource(m, circleSourceId, createCircleFeature(SAMSUNG_CENTER, SAMSUNG_RADIUS_METERS));
    addOrUpdateSource(m, haloInnerSourceId, createCircleFeature(SAMSUNG_CENTER, SAMSUNG_RADIUS_METERS));
    addOrUpdateSource(m, haloOuterSourceId, createCircleFeature(SAMSUNG_CENTER, SAMSUNG_RADIUS_METERS));
    addOrUpdateSource(m, sparkleSourceId, {
      type: 'FeatureCollection',
      features: createSparkleFeatures(SAMSUNG_CENTER, SAMSUNG_RADIUS_METERS * 1.06)
    });
    ...
    m.addLayer({
      id: 'samsung-taylor-site-circle-dash-outline',
      type: 'line',
      source: circleSourceId,
      paint: {
  ...
  const removeCircleLayer = useCallback(() => {
    ...
    ['samsung-taylor-site-circle-dash-outline', outlineId, haloInnerLayerId, haloOuterLayerId, sparkleLayerId, fillId].forEach(id => safeRemove(m, id));
    [sourceId, SAMSUNG_RING_IDS.haloInnerSource, SAMSUNG_RING_IDS.haloOuterSource, SAMSUNG_RING_IDS.sparkleSource].forEach(id => safeRemove(m, id, false));
  ...
  useEffect(() => {
    if (typeof window !== 'undefined') {
      window.samsungTaylorAnimationRef = {
        handlePlayPause,
        handleRestart,
        handleCleanup,
    ...
    return () => {
      if (typeof window !== 'undefined' && window.samsungTaylorAnimationRef) {
        delete window.samsungTaylorAnimationRef;
      }
    };
  ...
  useEffect(() => {
    ...
    const loadFrames = async () => {
      try {
        const responses = await Promise.all(
          PERIODS.map(async (period) => {
            try {
              ...
              console.warn('🛰️ Samsung CTA: Failed loading change frame', {
                file: period.file,
                error
              });
              return null;
        ...
        console.error('🛰️ Samsung CTA: Unexpected error loading animation frames', error);
    ...
    if (data) {
      ...
      if (!m.getLayer(LAYER_IDS.siteLabel)) {
        m.addLayer({
          ...
          type: 'symbol',
          source: LAYER_IDS.source,
          filter: ['==', ['get', 'zone'], 'samsung_taylor_site'],
          layout: {
...
export default SamsungTaylorChangeAnimation;
