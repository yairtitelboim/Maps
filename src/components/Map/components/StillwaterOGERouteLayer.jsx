 ...
 * This component handled route visualization from Stillwater to OG&E power facilities.
...
export default function StillwaterOGERouteLayer({ map, visible }) {
  ...
  useEffect(() => {
    ...
    // Wait for route source to be loaded (created by FirecrawlCall.jsx)
    ...
    // Listen for ready event from FirecrawlCall.jsx
    if (window.mapEventBus) {
      ...
      window.mapEventBus.on('stillwater-oge-route:ready', handleReady);
      ...
      return () => {
        if (window.mapEventBus) {
          window.mapEventBus.off('stillwater-oge-route:ready', handleReady);
        }
        if (rafRef.current) {
          cancelAnimationFrame(rafRef.current);
          rafRef.current = null;
