...
const SOURCE_ID = 'power-demand-source';
const LAYER_ID = 'power-demand-layer';
const DATA_URL = '/data/osm/corridor_power_demand.geojson';
...
const toPointFeature = (feature, index) => {
  ...
  switch (feature.geometry.type) {
    ...
    case 'MultiPolygon':
      try {
        ...
        console.warn('🏭 PowerDemandLayer: Failed to compute centroid', error);
      }
      break;
    default:
      break;
  ...
  return {
    ...
    properties: {
      ...feature.properties,
      __demandId: feature.properties?.id ?? `demand-${index}`
    }
  };
};

const PowerDemandLayer = ({ map, visible }) => {
  ...
  const loadData = useCallback(async () => {
    ...
    try {
      ...
      console.error('🏭 PowerDemandLayer: Error loading data:', error);
      return null;
    } finally {
  ...
  const addLayer = useCallback(async () => {
    ...
    if (!data) {
      console.warn('🏭 PowerDemandLayer: No data available; skipping layer creation.');
      return;
    }
...
export default PowerDemandLayer;
