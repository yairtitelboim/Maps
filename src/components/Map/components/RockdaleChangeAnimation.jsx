...
const PERIODS = [
  { id: '2017_2018', label: '2017 → 2018', file: '/data/rockdale_tx/rockdale_tx_2017_2018.geojson' },
  { id: '2018_2019', label: '2018 → 2019', file: '/data/rockdale_tx/rockdale_tx_2018_2019.geojson' },
  { id: '2019_2020', label: '2019 → 2020', file: '/data/rockdale_tx/rockdale_tx_2019_2020.geojson' },
  { id: '2020_2021', label: '2020 → 2021', file: '/data/rockdale_tx/rockdale_tx_2020_2021.geojson' },
  { id: '2021_2022', label: '2021 → 2022', file: '/data/rockdale_tx/rockdale_tx_2021_2022.geojson' },
  { id: '2022_2023', label: '2022 → 2023', file: '/data/rockdale_tx/rockdale_tx_2022_2023.geojson' },
  { id: '2023_2024', label: '2023 → 2024', file: '/data/rockdale_tx/rockdale_tx_2023_2024.geojson' },
  { id: '2024_2025', label: '2024 → 2025', file: '/data/rockdale_tx/rockdale_tx_2024_2025.geojson' }
];

const ROCKDALE_CENTER = [-97.015, 30.655];
const ROCKDALE_RADIUS_METERS = 9500;

const LAYER_IDS = {
  source: 'rockdale-geo',
  aoiFill: 'rockdale-aoi-fill',
  aoiOuter: 'rockdale-aoi-outer',
  industrialFill: 'rockdale-industrial-fill',
  roads: 'rockdale-roads',
  power: 'rockdale-power',
  siteLabel: 'rockdale-site-label'
};

const ROCKDALE_RING_IDS = {
  circleSource: 'rockdale-ring-source',
  haloInnerSource: 'rockdale-ring-halo-inner-source',
  haloOuterSource: 'rockdale-ring-halo-outer-source',
  sparkleSource: 'rockdale-ring-sparkles-source',
  circleFill: 'rockdale-ring-fill',
  circleOutline: 'rockdale-ring-outline',
  haloInnerLayer: 'rockdale-ring-halo-inner',
  haloOuterLayer: 'rockdale-ring-halo-outer',
  sparkleLayer: 'rockdale-ring-sparkles'
};
...
const RockdaleChangeAnimation = ({ map, data, changeData = null, visible = true }) => {
  // Archived: Texas-specific animation - disabled for Columbus migration
  // This animation is for Rockdale, TX and should not run for Columbus, OH
  return null;
  ...
  const addCalloutLayers = useCallback((callouts) => {
    ...
    const sourceId = 'rockdale-callouts-source';
    const layerId = 'rockdale-callouts-labels';
  ...
  const addPeriodToMap = useCallback((period, periodIndex = 0) => {
    ...
    const sourceId = `rockdale-change-${baseId}-source`;
    ...
    if (!m.getSource(backgroundSourceId)) {
      m.addSource(backgroundSourceId, {
        type: 'geojson',
        data: createCircleFeature(ROCKDALE_CENTER, ROCKDALE_RADIUS_METERS)
      });
    }
  ...
  const animateCircleLayers = useCallback(() => {
    ...
    const animate = () => {
      ...
      if (outlineId && m.getLayer(outlineId)) {
      m.setPaintProperty(outlineId, 'line-color', 'rgba(249, 115, 22, 0.85)');
  ...
  const addCircleLayer = useCallback(() => {
    ...
    const circleSourceId = 'rockdale-site-circle-source';
    const haloInnerSourceId = 'rockdale-site-halo-inner-source';
    const haloOuterSourceId = 'rockdale-site-halo-outer-source';
    const sparkleSourceId = 'rockdale-site-sparkles-source';

    const fillId = 'rockdale-site-circle-fill';
    const outlineId = 'rockdale-site-circle-outline';
    const haloInnerLayerId = 'rockdale-site-halo-inner';
    const haloOuterLayerId = 'rockdale-site-halo-outer';
    const sparkleLayerId = 'rockdale-site-sparkles';

    addOrUpdateSource(m, circleSourceId, createCircleFeature(ROCKDALE_CENTER, ROCKDALE_RADIUS_METERS));
    addOrUpdateSource(m, haloInnerSourceId, createCircleFeature(ROCKDALE_CENTER, ROCKDALE_RADIUS_METERS * 0.82));
    addOrUpdateSource(m, haloOuterSourceId, createCircleFeature(ROCKDALE_CENTER, ROCKDALE_RADIUS_METERS * 1.28));
    addOrUpdateSource(m, sparkleSourceId, {
      type: 'FeatureCollection',
      features: createSparkleFeatures(ROCKDALE_CENTER, ROCKDALE_RADIUS_METERS * 1.06)
    });
    ...
    m.addLayer({
      ...
      paint: {
        ...
        'fill-opacity': 0,
        'fill-outline-color': 'rgba(253, 186, 116, 0.32)'
      }
    });

    m.addLayer({
      id: 'rockdale-site-circle-dash-outline',
      type: 'line',
      source: circleSourceId,
      paint: {
        'line-color': 'rgba(253, 186, 116, 0.88)',
        'line-width': 3,
    ...
    m.addLayer({
      ...
      source: circleSourceId,
      paint: {
        'line-color': 'rgba(249, 115, 22, 0.68)',
        'line-width': 2,
  ...
  const removeCircleLayer = useCallback(() => {
    ...
    ['rockdale-site-circle-dash-outline', outlineId, haloInnerLayerId, haloOuterLayerId, sparkleLayerId, fillId].forEach(id => safeRemove(m, id));
    [sourceId, ROCKDALE_RING_IDS.haloInnerSource, ROCKDALE_RING_IDS.haloOuterSource, ROCKDALE_RING_IDS.sparkleSource].forEach(id => safeRemove(m, id, false));
  ...
  useEffect(() => {
    ...
    return () => {
      if (typeof window !== 'undefined' && window.rockdaleAnimationRef) {
        delete window.rockdaleAnimationRef;
      }
    };
  ...
  useEffect(() => {
    ...
    const loadFrames = async () => {
      try {
        const responses = await Promise.all(
          PERIODS.map(async (period) => {
            try {
              ...
              console.warn('🛰️ Rockdale CTA: Failed loading change frame', {
                file: period.file,
                error
              });
              return null;
        ...
        console.error('🛰️ Rockdale CTA: Unexpected error loading animation frames', error);
...
export default RockdaleChangeAnimation;
