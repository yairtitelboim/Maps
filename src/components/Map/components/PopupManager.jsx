...
import { formatWaterData, formatAIConsensusData, formatStartupData, formatTDLRData } from './PopupCards';
...
export const PopupManager = ({ map }) => {
  useEffect(() => {
    ...
    // Add hover effects for startup intelligence markers (popup handled by MarkerPopupManager)
    const startupLayers = ['startup-intelligence-markers', 'serp-startup-ecosystem-markers'];

    startupLayers.forEach(layerId => {
      // Add hover effect
      map.current.on('mouseenter', layerId, () => {
        map.current.getCanvas().style.cursor = 'pointer';
      });
      ...
      // Click handler removed - popup is now handled by MarkerPopupManager via event bus
      // This prevents duplicate popup creation
    });

    // Add hover effects for TDLR markers (popup handled by MarkerPopupManager)
    const tdlrLayers = ['tdlr-markers', 'tdlr-radius-circles'];

    tdlrLayers.forEach(layerId => {
      // Add hover effect
      map.current.on('mouseenter', layerId, () => {
        map.current.getCanvas().style.cursor = 'pointer';
      });
