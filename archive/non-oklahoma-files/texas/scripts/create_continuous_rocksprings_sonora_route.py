#!/usr/bin/env python3
"""
Create a continuous route line from Rocksprings → Sonora using routing API.
...
def main():
    # Coordinates: Rocksprings → Sonora (lat, lon format)
    ...
    print("Creating continuous route: Rocksprings → Sonora")
    print(f"Start point (Rocksprings): {rocksprings_coords}")
    print(f"End point (Sonora): {sonora_coords}")
    
    # Get route from OSRM
    route_geometry = get_osrm_route(rocksprings_osrm, sonora_osrm)
    ...
    # Save to file
    output_file = '../public/data/continuous_rocksprings_sonora_route.geojson'
    ...
    print(f"🎯 Route startpoint (Rocksprings): [{startpoint[0]}, {startpoint[1]}]")
