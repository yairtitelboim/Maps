#!/usr/bin/env python3
"""
Create a continuous route line from Junction → Utopia using routing API.
...
def main():
    # Coordinates: Junction → Utopia (lat, lon format)
    ...
    print("Creating continuous route: Junction → Utopia")
    print(f"Start point (Junction): {junction_coords}")
    print(f"End point (Utopia): {utopia_coords}")
    
    # Get route from OSRM
    route_geometry = get_osrm_route(junction_osrm, utopia_osrm)
    ...
    # Save to file
    output_file = '../public/data/continuous_junction_utopia_route.geojson'
