// Test the FIXED TRN (Transmission) filtering logic
const testData = {
    ...
    switch (categoryId) {
        ...
        case 'transmission':
            const transmissionNodes = nodes.filter(node => {
                const nodeText = node.toLowerCase();
                // Only include actual transmission operators, not power plants or local utilities
                return nodeText.includes('oncor') ||
                       nodeText.includes('tnmp') ||
                       nodeText.includes('texas-new mexico power') ||
                       // Exclude power plants and local utilities by checking node type
                       (nodeText.includes('transmission') && 
                        !nodeText.includes('power plant') && 
                        !nodeText.includes('water supply') &&
                        !nodeText.includes('hydroelectric') &&
                        !nodeText.includes('natural gas') &&
                        !nodeText.includes('combined cycle'));
            });
...
console.log('🧪 Testing FIXED TRN (Transmission) Filtering\n');

const filteredResponse = filterNodeLevelResponse(testData.nodeLevel, 'transmission');
...
if (nodeMatches) {
    console.log(`   Found ${nodeMatches.length} nodes in TRN category:`);
    nodeMatches.forEach((match, index) => {
        console.log(`   ${index + 1}. ${match}`);
    });
} else {
    console.log('   No nodes found in TRN category');
}
...
nodes.forEach((node, index) => {
    ...
    const matches = [];
    if (nodeText.includes('oncor')) matches.push('oncor');
    if (nodeText.includes('tnmp')) matches.push('tnmp');
    if (nodeText.includes('texas-new mexico power')) matches.push('texas-new mexico power');
    
    // Check the new transmission logic
    const hasTransmission = nodeText.includes('transmission');
    const hasPowerPlant = nodeText.includes('power plant');
    const hasWaterSupply = nodeText.includes('water supply');
    const hasHydroelectric = nodeText.includes('hydroelectric');
    const hasNaturalGas = nodeText.includes('natural gas');
    const hasCombinedCycle = nodeText.includes('combined cycle');
    
    if (hasTransmission) {
        if (hasPowerPlant || hasWaterSupply || hasHydroelectric || hasNaturalGas || hasCombinedCycle) {
            matches.push('transmission (EXCLUDED - power plant/utility)');
        } else {
            matches.push('transmission (INCLUDED - transmission operator)');
        }
    }
...
console.log('\n✅ FIXED TRN filtering test completed!');
