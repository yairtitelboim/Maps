// Test the TRN (Transmission) filtering logic
const testData = {
...
console.log('🧪 Testing TRN (Transmission) Filtering\n');
...
nodes.forEach((node, index) => {
    ...
    if (nodeText.includes('transmission')) matches.push('transmission');
    if (nodeText.includes('345 kv')) matches.push('345 kv');
    if (nodeText.includes('138 kv')) matches.push('138 kv');
    if (nodeText.includes('electric utility')) matches.push('electric utility');
    if (nodeText.includes('grid operator')) matches.push('grid operator');
    
    console.log(`   Node ${index + 1} (${nodeName}): ${matches.length > 0 ? matches.join(', ') : 'No matches'}`);
});

console.log('\n✅ TRN filtering test completed!');
