// Test the new summary generation functionality
const testData = {
...
function generateSummaryResponse(response) {
    ...
    nodes.forEach((node, index) => {
        ...
        if (criticalData.redundancy) {
            summary += `**🔄 Redundancy:** ${criticalData.redundancy}\n`;
        }
        if (criticalData.riskFactors) {
            summary += `**⚠️ Key Risks:** ${criticalData.riskFactors}\n`;
        }
...
const powerScores = (summary.match(/\*\*⚡ Power Score:\*\* \d+\/\d+/g) || []).length;
const stabilityScores = (summary.match(/\*\*🔧 Stability Score:\*\* \d+\/\d+/g) || []).length;
const capacities = (summary.match(/\*\*📊 Capacity:\*\*/g) || []).length;
const voltageLevels = (summary.match(/\*\*⚡ Voltage Level:\*\*/g) || []).length;

console.log(`   Power Scores: ${powerScores}`);
console.log(`   Stability Scores: ${stabilityScores}`);
console.log(`   Capacities: ${capacities}`);
console.log(`   Voltage Levels: ${voltageLevels}`);
