#!/usr/bin/env python3
"""
Create a continuous I-29 route centered on Kansas City using OSRM routing.
This creates a single LineString following the actual driving route along I-29.

Route: 500 miles northwest of KC → Kansas City → 500 miles southeast of KC
"""
...
def get_i29_waypoints(center_lat, center_lon, radius_miles):
    """
    Generate waypoints along I-29 corridor for routing.
    
    Args:
        center_lat: Kansas City latitude
        center_lon: Kansas City longitude
        radius_miles: Distance to extend northwest and southeast
    ...
    # I-29 runs roughly northwest-southeast
    # I-29 actually passes through the Kansas City metro area on the Missouri side
    
    # Northwest of Kansas City (towards Fargo, ND)
    # I-29 goes northwest from KC area
    northwest_bearing = 330  # 30 degrees west of north
    northwest_point = calculate_waypoint_coordinates(center_lat, center_lon, radius_miles, northwest_bearing)
    waypoints.append(northwest_point)
    
    # Kansas City area (I-29 passes through the metro)
    waypoints.append((center_lat, center_lon))
    
    # Southeast of Kansas City (towards New Orleans)  
    # I-29 continues southeast
    southeast_bearing = 150  # 30 degrees east of south
    southeast_point = calculate_waypoint_coordinates(center_lat, center_lon, radius_miles, southeast_bearing)
    waypoints.append(southeast_point)
...
def get_osrm_route(waypoints):
    ...
    try:
        print(f"Requesting I-29 route from OSRM...")
        print(f"Waypoints: {len(waypoints)} points")
        for i, (lat, lon) in enumerate(waypoints):
            if i == 0:
                print(f"  Start: {lat:.4f}, {lon:.4f} (≈500 miles northwest of KC)")
            elif i == len(waypoints) - 1:
                print(f"  End:   {lat:.4f}, {lon:.4f} (≈500 miles southeast of KC)")
            else:
...
def create_geojson_feature(route_geometry, metadata):
    ...
    feature = {
        ...
        "properties": {
            "name": "I-29 Kansas City Corridor",
            "description": "Continuous I-29 route covering 1000-mile corridor centered on Kansas City",
            "route_type": "continuous_navigation",
            "highway": "I-29",
            "center_point": "Kansas City, MO",
            "corridor_radius_miles": metadata['radius_miles'],
            "start_description": f"≈{metadata['radius_miles']} miles northwest of Kansas City",
            "end_description": f"≈{metadata['radius_miles']} miles southeast of Kansas City",
            "approximate_coverage": {
                "northwest": "Fargo, ND area",
                "center": "Kansas City, MO",
                "southeast": "New Orleans, LA area"
            }
        }
    }
    ...
    geojson_data = {
        ...
        "features": [feature],
        "metadata": {
            "route_name": "I-29 Kansas City Corridor (Continuous)",
            "description": "Single continuous route along I-29 covering 1000-mile corridor",
...
def save_geojson(geojson_data, output_path):
    ...
    try:
        ...
        print(f"✅ Saved continuous I-29 route to: {output_path}")
        ...
        print(f"   Start: [{start_point[0]:.4f}, {start_point[1]:.4f}] (Northwest)")
        print(f"   End:   [{end_point[0]:.4f}, {end_point[1]:.4f}] (Southeast)")
...
def main():
    ...
    """
    print("🛣️  I-29 Kansas City Continuous Route Creator")
    print("=" * 60)
    ...
    print(f"Corridor: {RADIUS_MILES} miles northwest + {RADIUS_MILES} miles southeast = {RADIUS_MILES * 2}-mile total")
    print()
    
    # Generate waypoints along I-29 corridor
    print("Calculating I-29 corridor waypoints...")
    waypoints = get_i29_waypoints(KC_LAT, KC_LON, RADIUS_MILES)
    ...
    output_path = os.path.join(script_dir, "..", "public", "data", "continuous_i29_kansas_city_corridor.geojson")
    
    success = save_geojson(geojson_data, output_path)
    
    if success:
        print(f"\n🎯 SUCCESS! Continuous I-29 route ready for mapping.")
        print(f"Load this file: {os.path.basename(output_path)}")
        print(f"\nThis creates a single continuous line following I-29 from:")
        print(f"  • Fargo, ND area (northwest)")
        print(f"  • Through Kansas City, MO (center)")  
        print(f"  • To New Orleans, LA area (southeast)")
        return True
    else:
        return False
