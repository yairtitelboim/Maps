...
def load_oge_facilities():
    ...
    try:
        ...
        print(f"❌ Error loading OG&E data: {e}")
        return []
...
def main():
    ...
    if not facilities:
        print("❌ No OG&E facilities found in data file")
        return False
    ...
    for facility in facilities:
        ...
        if abs(facility_lat - STILLWATER_COORDS[0]) < 0.0001 and abs(facility_lon - STILLWATER_COORDS[1]) < 0.0001:
            print(f"⚠️  Skipping {facility_name}: same location as Stillwater")
            continue
        ...
        route_description = f"Stillwater → {facility_name}"
