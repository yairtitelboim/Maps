#!/usr/bin/env python3
"""
Fetch I-29 highway segments centered on Kansas City, Missouri.
Creates a 1000-mile corridor (500 miles northwest + 500 miles southeast) of I-29.

This script uses the Overpass API to fetch actual I-29 highway segments
from OpenStreetMap, covering the major northwest-southeast corridor through the central United States.
"""
...
def build_i29_overpass_query(bbox):
    """
    Build comprehensive Overpass query for I-29 highway segments.
...
(
  // I-29 ways with various reference formats
  way["highway"]["ref"~"^(I-?29|Interstate 29)$"]({bbox_str});
  way["highway"]["ref"~"^29$"]["highway"~"^(motorway|trunk)$"]({bbox_str});
  
  // I-29 Business routes and branches
  way["highway"]["ref"~"^I-?29[EWNS]?$"]({bbox_str});
  way["highway"]["ref"~"^I-?29 Business$"]({bbox_str});
  way["highway"]["ref"~"^I-?29 BL$"]({bbox_str});
  way["highway"]["ref"~"^I-?29 BS$"]({bbox_str});
  
  // I-29 in relations (complete route coverage)
  relation["route"="road"]["ref"~"^(I-?29|Interstate 29)$"]({bbox_str});
  way(r)["highway"];
  
  // Motorways and trunks that might be I-29 but tagged differently
  way["highway"~"^(motorway|trunk)$"]["name"~".*29.*"]({bbox_str});
  way["highway"~"^(motorway|trunk)$"]["name"~".*Interstate.*29.*"]({bbox_str});
  
  // I-29 links and ramps
  way["highway"~"^(motorway_link|trunk_link)$"]["ref"~"29"]({bbox_str});
);
out geom;
"""
    return query.strip()

def fetch_i29_segments(bbox):
    """
    Fetch I-29 segments from Overpass API.
    ...
    """
    query = build_i29_overpass_query(bbox)
    ...
    try:
        ...
        if element_count == 0:
            print("⚠️  No I-29 segments found in the specified area")
            return None
...
def convert_osm_to_geojson(osm_data, metadata):
    ...
    for element in osm_data['elements']:
        if element['type'] == 'way' and 'geometry' in element:
            ...
            properties.update({
                'osm_id': element['id'],
                'osm_type': 'way',
                'route_name': 'I-29 Kansas City Corridor',
                'route_description': 'Interstate 29 highway segments in 1000-mile Kansas City corridor',
    ...
    geojson = {
        ...
        'metadata': {
            'route_name': 'I-29 Kansas City Corridor',
            'description': 'Interstate 29 highway segments covering 1000-mile corridor centered on Kansas City',
...
def save_geojson(geojson_data, output_path):
    ...
    try:
        ...
        print(f"✅ Saved I-29 route data to: {output_path}")
...
def main():
    ...
    """
    print("🛣️  I-29 Kansas City Corridor Fetcher")
    print("=" * 60)
    ...
    # Northeast: Minneapolis, MN area
    # Southwest: Oklahoma City, OK area
    print("Approximate coverage:")
    print("  Northwest: Fargo, ND area")
    print("  Southeast: New Orleans, LA area")
    print("  Northeast: Minneapolis, MN area") 
    print("  Southwest: Oklahoma City, OK area")
    print()
    
    # Fetch I-29 segments
    print("Fetching I-29 highway segments from OpenStreetMap...")
    osm_data = fetch_i29_segments(bbox)
    
    if not osm_data:
        print("❌ Failed to fetch I-29 data")
        return False
    ...
    if not geojson_data or not geojson_data['features']:
        print("❌ No valid I-29 features found")
        return False
    
    # Save to file
    script_dir = os.path.dirname(os.path.abspath(__file__))
    output_path = os.path.join(script_dir, "..", "public", "data", "i29_kansas_city_corridor.geojson")
    
    success = save_geojson(geojson_data, output_path)
    
    if success:
        print("\n🎯 SUCCESS! I-29 Kansas City corridor data ready for mapping.")
