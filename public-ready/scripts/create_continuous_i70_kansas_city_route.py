#!/usr/bin/env python3
"""
Create a continuous I-70 route centered on Kansas City using OSRM routing.
This creates a single LineString following the actual driving route along I-70.

Route: 500 miles west of KC → Kansas City → 500 miles east of KC
"""
...
def get_i70_waypoints(center_lat, center_lon, radius_miles):
    """
    Generate waypoints along I-70 corridor for routing.
    
    Args:
        center_lat: Kansas City latitude
        center_lon: Kansas City longitude
        radius_miles: Distance to extend east and west
    ...
    # I-70 runs roughly east-west, but with some deviation
    # Adjust bearings to follow the actual I-70 corridor
    
    # West of Kansas City (towards Denver)
    # I-70 goes slightly northwest from KC
    west_bearing = 285  # 15 degrees north of west
    west_point = calculate_waypoint_coordinates(center_lat, center_lon, radius_miles, west_bearing)
    waypoints.append(west_point)
    
    # Kansas City center
    waypoints.append((center_lat, center_lon))
    
    # East of Kansas City (towards St. Louis)  
    # I-70 goes slightly northeast from KC
    east_bearing = 75  # 15 degrees north of east
    east_point = calculate_waypoint_coordinates(center_lat, center_lon, radius_miles, east_bearing)
    waypoints.append(east_point)
...
def get_osrm_route(waypoints):
    ...
    try:
        print(f"Requesting I-70 route from OSRM...")
        print(f"Waypoints: {len(waypoints)} points")
        for i, (lat, lon) in enumerate(waypoints):
            if i == 0:
                print(f"  Start: {lat:.4f}, {lon:.4f} (≈500 miles west of KC)")
            elif i == len(waypoints) - 1:
                print(f"  End:   {lat:.4f}, {lon:.4f} (≈500 miles east of KC)")
            else:
...
def create_geojson_feature(route_geometry, metadata):
    ...
    feature = {
        ...
        "properties": {
            "name": "I-70 Kansas City Corridor",
            "description": "Continuous I-70 route covering 1000-mile corridor centered on Kansas City",
            "route_type": "continuous_navigation",
            "highway": "I-70",
            "center_point": "Kansas City, MO",
            "corridor_radius_miles": metadata['radius_miles'],
            "start_description": f"≈{metadata['radius_miles']} miles west of Kansas City",
            "end_description": f"≈{metadata['radius_miles']} miles east of Kansas City",
    ...
    geojson_data = {
        ...
        "features": [feature],
        "metadata": {
            "route_name": "I-70 Kansas City Corridor (Continuous)",
            "description": "Single continuous route along I-70 covering 1000-mile corridor",
...
def save_geojson(geojson_data, output_path):
    ...
    try:
        ...
        print(f"✅ Saved continuous I-70 route to: {output_path}")
        ...
        print(f"   Start: [{start_point[0]:.4f}, {start_point[1]:.4f}] (West)")
        print(f"   End:   [{end_point[0]:.4f}, {end_point[1]:.4f}] (East)")
...
def main():
    ...
    """
    print("🛣️  I-70 Kansas City Continuous Route Creator")
    print("=" * 60)
    ...
    print(f"Corridor: {RADIUS_MILES} miles west + {RADIUS_MILES} miles east = {RADIUS_MILES * 2}-mile total")
    print()
    
    # Generate waypoints along I-70 corridor
    print("Calculating I-70 corridor waypoints...")
    waypoints = get_i70_waypoints(KC_LAT, KC_LON, RADIUS_MILES)
    ...
    output_path = os.path.join(script_dir, "..", "public", "data", "continuous_i70_kansas_city_corridor.geojson")
    
    success = save_geojson(geojson_data, output_path)
    
    if success:
        print(f"\n🎯 SUCCESS! Continuous I-70 route ready for mapping.")
        print(f"Load this file: {os.path.basename(output_path)}")
        print(f"\nThis creates a single continuous line following I-70 from:")
        print(f"  • Denver, CO area (west)")
        print(f"  • Through Kansas City, MO (center)")  
        print(f"  • To St. Louis, MO area (east)")
        return True
    else:
        return False
