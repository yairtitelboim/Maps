#!/usr/bin/env python3
"""
Fetch I-35 highway segments centered on Kansas City, Missouri.
Creates a 400-mile corridor (200 miles north + 200 miles south) of I-35.

This script uses the Overpass API to fetch actual I-35 highway segments
from OpenStreetMap, covering the major corridor through the central United States.
"""
...
def build_i35_overpass_query(bbox):
    """
    Build comprehensive Overpass query for I-35 highway segments.
...
(
  // I-35 ways with various reference formats
  way["highway"]["ref"~"^(I-?35|Interstate 35)$"]({bbox_str});
  way["highway"]["ref"~"^35$"]["highway"~"^(motorway|trunk)$"]({bbox_str});
  
  // I-35 Business routes and branches
  way["highway"]["ref"~"^I-?35[EWNS]?$"]({bbox_str});
  way["highway"]["ref"~"^I-?35 Business$"]({bbox_str});
  way["highway"]["ref"~"^I-?35 BL$"]({bbox_str});
  way["highway"]["ref"~"^I-?35 BS$"]({bbox_str});
  
  // I-35 in relations (complete route coverage)
  relation["route"="road"]["ref"~"^(I-?35|Interstate 35)$"]({bbox_str});
  way(r)["highway"];
  
  // Motorways and trunks that might be I-35 but tagged differently
  way["highway"~"^(motorway|trunk)$"]["name"~".*35.*"]({bbox_str});
  way["highway"~"^(motorway|trunk)$"]["name"~".*Interstate.*35.*"]({bbox_str});
  
  // I-35 links and ramps
  way["highway"~"^(motorway_link|trunk_link)$"]["ref"~"35"]({bbox_str});
);
out geom;
"""
    return query.strip()

def fetch_i35_segments(bbox):
    """
    Fetch I-35 segments from Overpass API.
    ...
    """
    query = build_i35_overpass_query(bbox)
    ...
    try:
        ...
        if element_count == 0:
            print("⚠️  No I-35 segments found in the specified area")
            return None
...
def convert_osm_to_geojson(osm_data, metadata):
    ...
    for element in osm_data['elements']:
        if element['type'] == 'way' and 'geometry' in element:
            ...
            properties.update({
                'osm_id': element['id'],
                'osm_type': 'way',
                'route_name': 'I-35 Kansas City Corridor',
                'route_description': 'Interstate 35 highway segments in 400-mile Kansas City corridor',
    ...
    geojson = {
        ...
        'metadata': {
            'route_name': 'I-35 Kansas City Corridor',
            'description': 'Interstate 35 highway segments covering 400-mile corridor centered on Kansas City',
...
def save_geojson(geojson_data, output_path):
    ...
    try:
        ...
        print(f"✅ Saved I-35 route data to: {output_path}")
...
def main():
    ...
    """
    print("🛣️  I-35 Kansas City Corridor Fetcher")
    print("=" * 60)
    ...
    # South: Laredo, TX area  
    # East: St. Louis, MO area
    # West: Denver, CO area
    print("Approximate coverage:")
    print("  North: Minneapolis, MN area")
    print("  South: Laredo, TX area")
    print("  East: St. Louis, MO area") 
    print("  West: Denver, CO area")
    print()
    
    # Fetch I-35 segments
    print("Fetching I-35 highway segments from OpenStreetMap...")
    osm_data = fetch_i35_segments(bbox)
    
    if not osm_data:
        print("❌ Failed to fetch I-35 data")
        return False
    ...
    if not geojson_data or not geojson_data['features']:
        print("❌ No valid I-35 features found")
        return False
    
    # Save to file
    script_dir = os.path.dirname(os.path.abspath(__file__))
    output_path = os.path.join(script_dir, "..", "public", "data", "i35_kansas_city_corridor.geojson")
    
    success = save_geojson(geojson_data, output_path)
    
    if success:
        print("\n🎯 SUCCESS! I-35 Kansas City corridor data ready for mapping.")
