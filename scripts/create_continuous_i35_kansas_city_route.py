#!/usr/bin/env python3
"""
Create a continuous I-35 route centered on Kansas City using OSRM routing.
This creates a single LineString following the actual driving route along I-35.

Route: 200 miles north of KC → Kansas City → 200 miles south of KC
"""
...
def get_i35_waypoints(center_lat, center_lon, radius_miles):
    """
    Generate waypoints along I-35 corridor for routing.
    ...
    # I-35 runs roughly north-south, but with some deviation
    # Adjust bearings to follow the actual I-35 corridor
    
    # North of Kansas City (towards Des Moines)
    # I-35 goes slightly northeast from KC
    north_bearing = 15  # 15 degrees east of north
    ...
    # South of Kansas City (towards Oklahoma City)  
    # I-35 goes slightly southwest from KC
    south_bearing = 195  # 15 degrees west of south
...
def get_osrm_route(waypoints):
    ...
    try:
        print(f"Requesting I-35 route from OSRM...")
        print(f"Waypoints: {len(waypoints)} points")
        for i, (lat, lon) in enumerate(waypoints):
            if i == 0:
                print(f"  Start: {lat:.4f}, {lon:.4f} (≈200 miles north of KC)")
            elif i == len(waypoints) - 1:
                print(f"  End:   {lat:.4f}, {lon:.4f} (≈200 miles south of KC)")
            else:
...
def create_geojson_feature(route_geometry, metadata):
    ...
    feature = {
        ...
        "properties": {
            "name": "I-35 Kansas City Corridor",
            "description": "Continuous I-35 route covering 1000-mile corridor centered on Kansas City",
    ...
    geojson_data = {
        ...
        "features": [feature],
        "metadata": {
            "route_name": "I-35 Kansas City Corridor (Continuous)",
            "description": "Single continuous route along I-35 covering 1000-mile corridor",
...
def save_geojson(geojson_data, output_path):
    ...
    try:
        ...
        print(f"✅ Saved continuous I-35 route to: {output_path}")
...
def main():
    ...
    """
    print("🛣️  I-35 Kansas City Continuous Route Creator")
    print("=" * 60)
    ...
    # Generate waypoints along I-35 corridor
    print("Calculating I-35 corridor waypoints...")
    waypoints = get_i35_waypoints(KC_LAT, KC_LON, RADIUS_MILES)
    ...
    output_path = os.path.join(script_dir, "..", "public", "data", "continuous_i35_kansas_city_corridor.geojson")
    
    success = save_geojson(geojson_data, output_path)
    
    if success:
        print(f"\n🎯 SUCCESS! Continuous I-35 route ready for mapping.")
        print(f"Load this file: {os.path.basename(output_path)}")
        print(f"\nThis creates a single continuous line following I-35 from:")
        print(f"  • Minneapolis, MN area (north)")
        print(f"  • Through Kansas City, MO (center)")  
        print(f"  • To Laredo, TX area (south)")
        return True
    else:
        return False
