// Test the UTL (Local Utilities) filtering logic
const testData = {
...
console.log('🧪 Testing UTL (Local Utilities) Filtering\n');
...
nodes.forEach((node, index) => {
    ...
    if (nodeText.includes('utility')) matches.push('utility');
    if (nodeText.includes('distribution')) matches.push('distribution');
    
    console.log(`   Node ${index + 1} (${nodeName}): ${matches.length > 0 ? matches.join(', ') : 'No matches'}`);
});

console.log('\n✅ UTL filtering test completed!');
